 ...
 */
registerMooseObject("Neutron_TransportApp", FissionTerm2G);
...
FissionTerm2G::FissionTerm2G(const InputParameters & parameters) : Kernel(parameters),
...
Real
FissionTerm2G::computeQpResidual()
{
	
  return -(_v/_k)*_chi*(  _sigmaF * _u[_qp] * _test[_i][_qp] );
  //return _sigmaF * _test[_i][_qp];
}

Real
FissionTerm2G::computeQpJacobian()
{
  return -(_v/_k)*_chi*(  _sigmaF * _phi[_j][_qp] * _test[_i][_qp] );
  //return 0.0;
}