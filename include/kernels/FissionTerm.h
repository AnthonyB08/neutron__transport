...
/// FissionTerm class inherits from Kernel class
class FissionTerm : public Kernel
{
public:
  /**
  ...
  FissionTerm(const InputParameters & parameters);
...
protected:
  ...
  /// The variables which holds the value for the FissionTerm coefficient
  const Real _sigmaF;
  const Real _k;
  const Real _v;
};